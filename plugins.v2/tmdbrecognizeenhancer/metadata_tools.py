...
class RenameFieldRegistry:
    ...
    VALUE_GUIDES: Dict[str, Dict[str, str]] = {
        ...
        "probe_subtitle_languages": {"type": "组合文本", "values": "简体、繁体、中文、日语、英语、韩语等单一语言；多条轨道用顿号连接去重。", "logic": "以轨道 language 标签为准，标题只用于细分简体/繁体；“简日双语”轨（language=chi）记为简体。硬字幕或无字幕轨时为空。"},
        "probe_subtitle_titles": {"type": "组合文本", "values": "字幕轨原始标题，例如 简日双语、繁日雙語；轨道未设置标题时为空。", "logic": "ffprobe tags.title 原文去重后用顿号连接，用于核对语言识别依据。"},
        "probe_subtitle_track_labels": {"type": "组合文本", "values": "每条字幕轨归一后的简写标签，例如 简日、繁日。", "logic": "综合 language/title 把每轨压缩成简写（简日双语 → 简日），去重后按语言顺序连接，便于命名引用。"},
        "probe_subtitle_mapped": {"type": "组合/映射文本", "values": "简繁日内封、多国字幕等。", "logic": "规则按行序首条命中，命中即为规则结果；未命中时回退为自动语言组合（超过 3 种语言归为多国内封）。精确与包含匹配同时按组合标签和单语言并集比较，>=N 按单语言数量。"},
