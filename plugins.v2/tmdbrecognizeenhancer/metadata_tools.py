...
class RenameFieldRegistry:
    ...
    VALUE_GUIDES: Dict[str, Dict[str, str]] = {
        ...
        "probe_subtitle_languages": {"type": "组合文本", "values": "简体、繁体、中文、日语、英语、韩语等；多种语言用顿号连接。", "logic": "只检查容器内独立字幕流的 language/title；硬字幕或无字幕轨时为空。"},
        "probe_subtitle_profile": {"type": "组合文本", "values": "简体内封、简繁内封、简繁日内封等。", "logic": "按识别到的字幕语言顺序压缩成命名友好的组合；没有可识别语言时为空。"},
        "probe_subtitle_mapped": {"type": "用户映射文本", "values": "由字幕组合映射规则决定，例如 简繁内封。", "logic": "仅在实际字幕语言集合精确命中用户规则时产生。"},
