...
class RenameFieldRegistry:
    ...
    VALUE_GUIDES: Dict[str, Dict[str, str]] = {
        ...
        "probe_subtitle_languages": {"type": "组合文本", "values": "简体、繁体、中文、日语、英语、韩语及双语轨组合标签（简日、繁日等）；多条轨道用顿号连接。", "logic": "只检查容器内独立字幕流的 language/title；“简日双语”这类双语轨显示为组合标签；硬字幕或无字幕轨时为空。"},
        "probe_subtitle_titles": {"type": "组合文本", "values": "字幕轨原始标题，例如 简日双语、繁日雙語；轨道未设置标题时为空。", "logic": "ffprobe tags.title 原文去重后用顿号连接，用于核对语言识别依据。"},
        "probe_subtitle_profile": {"type": "组合文本", "values": "简体内封、简繁内封、简繁日内封、多国内封等。", "logic": "把检测语言（组合标签先还原成原子语言）取并集后压缩；超过 3 种语言显示多国内封。"},
        "probe_subtitle_mapped": {"type": "用户映射文本", "values": "由字幕组合映射规则决定，例如 简繁日内封、多国字幕。", "logic": "规则按行序首条命中；精确与包含匹配同时按组合标签和原子语言并集比较，>=N 按原子语言数量。"},
