...
def test_candidate_rich_telegram_edits_blocks_and_replaces_photo(monkeypatch):
    ...
    class FakeRequestUtils:
        ...
        def post_res(self, url, **kwargs):
            ...
            if kwargs.get("data"):
                rich_message = json.loads(
                    kwargs["data"]["rich_message"],
                )
            else:
                rich_message = kwargs["json"]["rich_message"]
            blocks = rich_message.get("blocks") or []
            has_photo = any(
                block.get("type") == "photo"
                for block in blocks
                if isinstance(block, dict)
            )
            photo_count = sum(
                1 for item_url, item_kwargs in captured
                if (
                    item_url.endswith("/editMessageText")
                    and item_kwargs.get("data")
                )
            )
            response_blocks = []
            if has_photo:
                response_blocks = [{
                    "type": "photo",
                    "photo": [{
                        "file_id": f"photo-file-{photo_count}",
                        "file_unique_id": f"photo-unique-{photo_count}",
                        "width": 1000,
                        "height": 1500,
                    }],
                }]
    ...
    monkeypatch.setattr(module, "RequestUtils", FakeRequestUtils)
    plugin._fetch_candidate_poster = Mock(
        side_effect=[b"collage-image", b"poster-image"],
    )
    instance = SimpleNamespace(
        _telegram_token="secret-token",
        _determine_target_chat_id=lambda userid, chat_id: chat_id or "123",
    ...
    first_result = plugin._send_candidate_rich_telegram(
        instance=instance,
        rich_markdown="# 候选",
        rich_blocks=[{
            "type": "heading",
        ...
        buttons=[],
        image="https://image/collage.jpg",
        original_message_id=321,
        original_media_message_id=320,
        original_chat_id="654",
    )
    assert first_result["success"] is True
    second_result = plugin._send_candidate_rich_telegram(
        instance=instance,
        rich_markdown="# 候选",
        rich_blocks=[{
            "type": "heading",
        ...
        buttons=[],
        image="https://image/poster.jpg",
        original_message_id=321,
        original_photo_unique_id=first_result["photo_unique_id"],
        original_image_digest=first_result["image_digest"],
        original_chat_id="654",
    )
...
def test_candidate_rich_telegram_falls_back_when_media_field_is_unsupported(
    ...
    class FakeRequestUtils:
        ...
        def post_res(self, url, **kwargs):
            ...
            if "blocks" in rich_message:
                return FakeResponse({
                    "ok": False,
                    "description": "Bad Request: blocks are unsupported",
                })
            return FakeResponse({
                "ok": True,
                "result": {
...
def test_candidate_rich_telegram_keeps_rich_result_with_stale_photo_identity(
    ...
    monkeypatch.setattr(module, "RequestUtils", FakeRequestUtils)
    plugin._fetch_candidate_poster = Mock(return_value=b"new-poster-image")
    instance = SimpleNamespace(
        _telegram_token="secret-token",
        _determine_target_chat_id=lambda userid, chat_id: chat_id or "123",
    )
    ...
    result = plugin._send_candidate_rich_telegram(
        ...
        image="https://image/poster.jpg",
        original_message_id=321,
        original_photo_unique_id="same-photo",
        original_image_digest="old-image-digest",
        original_chat_id="654",
    )
...
def test_takeover_deduplicates_hidden_native_type_differences(monkeypatch):
    module = _load_plugin(monkeypatch)
    plugin = _plugin_with_runtime(module, SimpleNamespace())
    plugin._config.update({
        "notification_enhancer_enabled": True,
    ...
    plugin._handle_notice_message(SimpleNamespace(event_data={
        **visible,
        # MP 的另一条发送路径可能只保留成功 ctype，mtype 则落到手动处理。
        "mtype": "手动处理",
        "ctype": "organizeSuccess",
    }))
