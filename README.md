...
这是一个 MoviePilot V2 媒体整理增强插件。它在不修改 MoviePilot 或 Rust 源码的前提下，扩展候选识别、集数归一化、识别字段、命名上下文、制作组编排、最终重命名和运行诊断。
...
- 浏览 MP 内置制作组并设置动漫、真人电视剧或未分类，作为 TMDB 候选题材证据
- 浏览 MP 内置识别规则并通过插件覆盖层校正字段，不修改 MP/Rust 文件
